...
export async function POST(req: NextRequest) {
  ...
  const supabase = supabaseAdmin();
  const tokenRow = await consumeToken(supabase, token, 'school_setup');
  ...
  // Normalize the account email before storing it so login comparisons remain consistent.
  const normalizedEmail = school.email.toLowerCase().trim();

  const { data: admin, error: insertError } = await supabase
    .from('school_admins')
    .insert({ school_id: school.id, name, email: normalizedEmail, password_hash })
    .select('id, name, email')
    .single();

  if (insertError) {
    const message =
      insertError.code === '23505' ? 'An admin account for this email already exists.' : insertError.message;
